...
export default function SeriesTreeToc({
  ...
  return (
    <div
      className={`relative bg-slate-700/50 ${isExpanded ? "rounded-t-md" : "rounded-md"} p-4 mb-8`}
    >
      <div
        className="flex justify-between items-center cursor-pointer"
        onClick={toggleExpansion}
      >
        <h2 className="text-xl font-bold text-white">{seriesTitle}</h2>
      ...
      >
        <ul className="space-y-2 pt-0 pb-4 px-4">
          {" "}
          {/* 내부 패딩 조정 */}
